    ...
    for (Entry entry : entries) {
      if (!fields.contains(entry.getMimeType())) {
        CompositeMetadataCodec
            .encodeAndAddMetadataWithCompression(metadata, ByteBufAllocator.DEFAULT, entry.getMimeType(), entry.getContent());
      }
    }

    final Payload newPayload;
    if (payload instanceof ByteBufPayload) {
      newPayload = ByteBufPayload
          .create(payload.data().retain(), metadata.retain());
      payload.release();
    } else {
